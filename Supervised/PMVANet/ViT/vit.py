...
def build_model(
    ...
    # x = tf.keras.layers.Input(shape=(image_size_tuple[0], image_size_tuple[1], 1))
    ...
    # x = tf.keras.layers.Input(shape=(512, 1024))

    # y = layers.ClassToken(name="class_token")(y)
    
    x = tf.keras.layers.Input(shape=(image_size_tuple[0], image_size_tuple[1]))

    y = layers.AddPositionEmbs(name="Transformer/posembed_input")(x)
